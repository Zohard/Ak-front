export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Call the NestJS backend for collection animes
    ...
    const animes = (response.data || response.animes || response || []).map((item: any) => ({
      ...item,
    ...
    let errorMessage = error.response?.data?.message || error.message || 'Failed to fetch collection animes'
