<template>
  <div class="manga-reviews-page">
    <!-- Hero Section -->
    <div class="hero-section bg-gradient-to-r from-green-600 to-emerald-600 text-white py-16">
      <div class="container mx-auto px-4">
        <div class="text-center">
          <h1 class="text-4xl md:text-5xl font-bold mb-4 flex items-center justify-center gap-4">
            <Icon name="heroicons:book-open" class="w-12 h-12 text-green-200" />
Critiques de Mangas
          </h1>
          <p class="text-xl md:text-2xl mb-6 opacity-90">
            Découvrez les avis de la communauté sur vos mangas préférés
          </p>
          <div class="flex flex-wrap justify-center gap-4 text-sm md:text-base">
            <div class="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
              <span class="font-semibold">{{ totalReviews }}</span> critiques de mangas
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="container mx-auto px-4 py-8">
      <!-- Top Reviews -->
      <div class="mb-8">
        <TopReviews :limit="6" default-type="manga" />
      </div>
      ...
      <div class="mb-8">
        <ReviewFiltersComponent
          ...
          search-placeholder="Titre, contenu, manga..."
          media-type="manga"
      ...
      <div v-if="loading" class="flex items-center justify-center py-12">
        ...
        <span class="ml-3 text-gray-600 dark:text-gray-300">Chargement des critiques de mangas...</span>
      </div>
      ...
      <div v-else class="text-center py-12">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">
          Aucune critique de manga trouvée
        </h3>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          {{ hasActiveFilters ? 'Essayez de modifier vos filtres.' : 'Aucune critique de manga disponible pour le moment.' }}
        </p>
...
// Page Meta
definePageMeta({
  title: 'Critiques de Mangas',
  description: 'Découvrez les critiques et avis de la communauté sur vos mangas préférés'
})

// SEO
useSeoMeta({
  title: 'Critiques de Mangas - Anime-Kun',
  description: 'Découvrez les critiques et avis de la communauté sur vos mangas préférés. Notes, commentaires et recommandations par des passionnés.',
  ogTitle: 'Critiques de Mangas - Anime-Kun',
  ogDescription: 'Découvrez les critiques et avis de la communauté sur vos mangas préférés.'
})
...
const filters = ref<ReviewFilters>({
  search: '',
  type: 'manga', // Always manga for this page
  rating: null,
  sort: 'recent',
  author: '',
  tags: []
...
const updateFilters = (newFilters: ReviewFilters) => {
  filters.value = { ...newFilters, type: 'manga' } // Ensure type stays manga
  loadReviews()
