  ...
  <div class="admin-layout">
    ...
    <aside class="admin-sidebar">
      ...
      <div class="sidebar-section">
        ...
        <NuxtLink to="/admin/business" class="sidebar-link active">
      ...
      <div class="sidebar-section">
        ...
        <div class="quick-links">
          ...
          <a href="/admin/business/create" class="quick-link">/admin/business/create</a>
    ...
    <main class="admin-main">
      <div class="section-header">
        ...
        <NuxtLink to="/admin/business/create" class="btn-primary">Créer une entreprise</NuxtLink>
      </div>
    ...
    <div v-else-if="error" class="card p-4 text-center text-red-600">
      <p>{{ error }}</p>
      <button class="btn-secondary mt-2" @click="load">Réessayer</button>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="b in items" :key="b.idBusiness">
            <td>{{ b.idBusiness }}</td>
            <td>
              <NuxtLink :to="`/admin/business/${b.idBusiness}`" class="link">
                {{ b.denomination }}
              </NuxtLink>
            </td>
            <td>{{ b.type || '—' }}</td>
            ...
            <td class="flex gap-2">
              ...
              <button class="btn-small danger" @click="remove(b)">
                Supprimer
              </button>
            </td>
          </tr>
...
const load = async () => {
  try {
    ...
    // Only add page if it's not 1 (API defaults to page 1)
    ...
    const response = await $fetch<{items: Business[], pagination: any}>(`${config.public.apiBase}/api/admin/business`, {
      params,
    ...
    if (page.value === 1 && !filters.search && !filters.type && typeof filters.statut !== 'number') {
      success(`${totalItems.value} entreprises chargées avec succès`, { duration: 3000 })
    }
    
  } catch (err: any) {
    error.value = err?.data?.message || 'Erreur lors du chargement des entreprises'
...
const toggleStatus = async (b: Business) => {
  try {
    ...
    success(`${b.denomination} a été ${newStatus === 1 ? 'activé' : 'désactivé'}`)
    load()
  } catch (err) {
...
const remove = async (b: Business) => {
  ...
  try {
    ...
    success(`"${b.denomination}" a été supprimé`)
    load()
  } catch (err) {
...
const getStatusClass = (statut: number) => {
  ...
  if (statut === 2) return `${baseClass} chip-yellow`
  return `${baseClass} chip-gray`
}

const getStatusText = (statut: number) => {
  if (statut === 1) return 'Affichée'
  if (statut === 2) return 'En attente'
  return 'Bloquée'
}
