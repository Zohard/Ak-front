  ...
  <div class="admin-layout">
    ...
    <aside class="admin-sidebar">
      ...
      <div class="sidebar-section">
        ...
        <NuxtLink to="/admin/animes" class="sidebar-link active">
          <Icon name="heroicons:film" class="w-4 h-4" />
          Animes
        </NuxtLink>
        <NuxtLink to="/admin/mangas" class="sidebar-link">
      ...
      <div class="sidebar-section">
        ...
        <div class="quick-links">
          ...
          <a href="/admin/animes/create" class="quick-link">/admin/animes/create</a>
    ...
    <div class="card p-4 mb-4 flex gap-3 items-end">
      <div>
        ...
        <input v-model="filters.search" class="form-input" placeholder="Titre..." @keyup.enter="loadAnimes" />
      </div>
      <div>
      ...
      <button class="btn-secondary" @click="loadAnimes">Appliquer</button>
    </div>
    ...
    <div v-else-if="error" class="card p-4 text-center text-red-600">
      <p>{{ error }}</p>
      <button class="btn-secondary mt-2" @click="loadAnimes">Réessayer</button>
    </div>
    ...
    <div v-else class="card overflow-x-auto">
      <table class="table">
        ...
        <tbody>
          <tr v-for="anime in animes" :key="anime.idAnime">
            <td>{{ anime.idAnime }}</td>
            <td>
              <NuxtLink :to="`/admin/animes/${anime.idAnime}`" class="link">
                {{ anime.titre }}
              </NuxtLink>
            </td>
            <td>{{ anime.annee || '—' }}</td>
            <td>{{ anime.studio || '—' }}</td>
            <td>
              <span :class="getStatusClass(anime.statut)">
                {{ getStatusText(anime.statut) }}
              </span>
            </td>
            <td class="flex gap-2">
              <button class="btn-small" @click="toggleStatus(anime)">
                {{ anime.statut === 1 ? 'Dépublier' : 'Publier' }}
              </button>
              <button class="btn-small danger" @click="deleteAnime(anime)">
                Supprimer
              </button>
            </td>
          </tr>
...
const loadAnimes = async () => {
  try {
    ...
    if (filters.studio?.trim()) params.studio = filters.studio.trim()
    if (typeof filters.statut === 'number') params.statut = filters.statut
    
    const response = await $fetch<{items: Anime[], pagination: any}>(`${config.public.apiBase}/api/admin/animes`, {
      params,
      headers: authStore.getAuthHeaders() as Record<string, string>
    })
    
    animes.value = response.items || []
    ...
    if (currentPage.value === 1 && !filters.search && !filters.annee && !filters.studio && typeof filters.statut !== 'number') {
      success(`${totalItems.value} animes chargés avec succès`, { duration: 3000 })
    }
    
  } catch (err: any) {
    error.value = err?.data?.message || 'Erreur lors du chargement des animes'
...
const toggleStatus = async (anime: Anime) => {
  try {
    const newStatus = anime.statut === 1 ? 0 : 1
    await $fetch(`${config.public.apiBase}/api/admin/animes/${anime.idAnime}/status`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', ...authStore.getAuthHeaders() as any },
      body: { statut: newStatus }
    })
    success(`${anime.titre} a été ${newStatus === 1 ? 'publié' : 'dépublié'}`)
    loadAnimes()
  } catch (err) {
...
const deleteAnime = async (anime: Anime) => {
  if (!confirm(`Supprimer "${anime.titre}" ?`)) return
  
  try {
    await $fetch(`${config.public.apiBase}/api/admin/animes/${anime.idAnime}`, {
      method: 'DELETE',
      headers: authStore.getAuthHeaders() as any
    })
    success(`"${anime.titre}" a été supprimé`)
    loadAnimes()
  } catch (err) {
