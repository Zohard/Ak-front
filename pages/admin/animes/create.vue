<template>
  <div class="main-content max-w-3xl">
    <h1 class="section-title mb-4">Créer un anime</h1>
    ...
    <form class="card p-5 space-y-4" @submit.prevent="submit">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input v-model.number="form.annee" type="number" min="1900" class="form-input w-full" />
        </div>
        <div>
          <label class="block text-sm mb-1">Nombre d'épisodes</label>
          <input v-model.number="form.nbEp" type="number" min="0" class="form-input w-full" />
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label class="block text-sm mb-1">Studio</label>
          <input v-model="form.studio" class="form-input w-full" />
        </div>
        <div>
          <label class="block text-sm mb-1">Réalisateur</label>
          <input v-model="form.realisateur" class="form-input w-full" />
        </div>
      </div>
      <div>
      ...
      <div class="flex gap-3">
        ...
        <NuxtLink to="/admin/animes" class="btn-secondary">Annuler</NuxtLink>
      </div>
    </form>
  </div>
</template>
...
const submit = async () => {
  try {
    await $fetch(`${config.public.apiBase}/api/admin/animes`, {
      method: 'POST',
